...
public class Leap {
	...
	public static void main(String[] args) throws Exception {
		...
		while (yolo.hasNext()) {
			...
			String[] nums = yolo.nextLine().substring(1)
					.replaceAll("[\\[\\] ]", "").split(",");
		...
		String dictkey = perfectLetters.keySet().toString()
				.replaceAll("[@\\[\\] ,]", "");
		out.println(dictkey);
		...
		while (true) {
			while ((in.available() == 0 && controller.frame().hands().count() < 2)
					|| innumbermode == true) {
				Thread.sleep(3);
			...
			if (!curr.hands().isEmpty()) {
				...
				gesture[20] = (abs(currenthand.palmNormal().roll()) > PI / 4
						? 1
						: 0);
				int fingernum = 0;
				for (Finger x : currenthand.fingers()) {
					...
					for (int u = 0; u < 3; u++) {
						gesture[fingernum * 3 + u] = x.direction().get(u)
								- palm.get(u);
					}
	...
	public static Map<String, List<Double>> extension(
			Map<String, List<Double>> set, double[] match) {
	...
	public static String bestMatchArithmetic(Map<String, List<Double>> set,
			double[] match) {
		String bestmatch = "";
			...
			for (int x = 0; x < 5; x++) {
				...
				for (int i = 0; i < 3; i++) {
					if (set.get(key).get(i) * match[i] > 0)
						diff += pow(abs(set.get(key).get(x * 3 + i)
								- match[x * 3 + i]), 5);
					else
						diff += pow(abs(set.get(key).get(x * 3 + i)
								+ match[x * 3 + i]), 5);
